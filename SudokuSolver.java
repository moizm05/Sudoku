...
public class SudokuSolver {
	...
	return board[x][y] == 0;//Returns 0 as value 
    }

    //To check if the game is completed or not.
    public static boolean isComplete(int board[][]){
	for(int i = 0; i < 9; i++){//For loop to run through all spots on board
	    for(int j = 0; j < 9; j++){//For loop to run through all spots on board
		if(board[i][j] == 0) return false;//Returns False if board is not complete
	    }
	}
	return true;//Returns true if it is complete 
    }

    //To check if current move is valid or not.
    public static boolean isMoveValid(int x, int y, int move, int board[][]){

	//To check if the number entered is valid or not.Return False if not
	if(x < 0 || y < 0 || x > 9 || y > 9) return false;

	//To check the column constraint.Return False if not
	for(int i = 0; i < 9; i++){
	    if(board[i][y] == move) return false;
	}

	//To check the row constraint.Return False if not
	for(int i = 0; i < 9; i++){
	    if(board[x][i] == move) return false;
	}

	//To check the sub-box.Return False if not
	...
	//Get the next empty point on board
    ...
    //Returns right and wrong answers back to the main class
